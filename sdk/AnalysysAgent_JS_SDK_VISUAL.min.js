(function (factory) {
  ...
  function domParentList(ele) {
    ...
    while (parent != null) {
      ...
      var classNameList = Util$1.paramType(parent.className) !== 'String' ? [] : parent.className.split(' ');
  ...
  function pathContrast(clickEleObj, eventEleObj) {
    ...
    if (clickPathArray.length > eventPathArray.length) {
      ...
      clickPathArray = clickPathArray.splice(lengthNum); // 点击的为子节点则不校验点击元素位置
  ...
  function parseEvent(path) {
    ...
    var eleList = Util$1.selectorAllEleList(baseEle.elePath); // document.querySelectorAll(baseEle.elePath)
    // var anchors = [];
  ...
  function setEventTemp(config) {
    ...
    var temp = eventTemp.replace('{ELETAGNAME}', config.tagName || '').replace('{CHANGETEXT}', config.isChange ? '编辑事件' : '创建事件').replace('{EVENTID}', config.appEventId || '').replace('{EVENTNAME}', config.appEventName || '').replace('{ISPAGE}', config.isAll === 1 ? '' : 'checked="checked"').replace('{ISPAGECLASS}', config.isAll === 1 ? '' : 'v-radio-checked').replace('{ISTEXTDISPLAY}', config.content !== '' ? '' : 'style="visibility:hidden"').replace('{ISTEXT}', config.isText !== '' ? 'checked="checked"' : '').replace('{ISTEXTCLASS}', config.isText !== '' ? 'v-radio-checked' : '').replace('{EVENTTEXT}', text !== '' ? '限制在文本： ' + text : '无文本');
    return temp;
  }
  ...
  function getDomainFromUrl$1(url) {
    ...
    let offsetPath = 0;
    let len = url.length;
    let i = 0; // Find end offset of domain
  ...
  function setUUId() {
    ...
    var id = 'JS' + Util$1.MD5(timeRandom.toString(), 32) + Util$1.MD5(timeRandom.toString(), 32).slice(0, 4); // var storageId = Storage.getCookie("ARK_ID")
  ...
  function isLogin() {
    return !!(getAliasId() || Storage$1.getLocal('ARK_TRACK_LOGIN'));
  }
  ...
  function getScreenWidth() {
    var width = Util$1.paramType(Number(window.screen.width)) === 'Number' ? Number(window.screen.width) : 0;
    return width;
  }

  function getScreenHeight() {
    var height = Util$1.paramType(Number(window.screen.height)) === 'Number' ? Number(window.screen.height) : 0;
    return height;
  }
  /**
   * UTM参数控制逻辑
  ...
  function loadVisitorSDK() {
    Util$1.addScript('AnalysysAgent_JS_SDK_VISUAL', visitorConfig.SDKFileDirectory);
    window.ARK_VISUAL = {
  ...
  function addElesListener(e) {
    ...
    var ele = e.target || e.srcElement; // var elePath = domParentList(ele)
  ...
  function getVisitorEvent() {
    ...
    var url = window.location.protocol + '//' + window.location.host + window.location.pathname + hash; // var noProUrl = '//' + window.location.host + window.location.pathname + hash

    getPoint(url); // getPoint(noProUrl, true)
  }
  ...
  function pcMouseOver(e) {
    ...
    Util$1.addEvent(ele, 'mousedown', addElesListener);
  }

  function pcMouseOut(e) {
    var ele = e.target; // eventEle = null

    Util$1.removeEvent(ele, 'mousedown', addElesListener);
  }
  /**
   *添加点击监听
   *在dom加载完毕后绑定
  ...
  function addEventListener() {
    ...
    if (Util$1.deviceType() === 'desktop') {
      Util$1.addEvent(document, 'mouseover', pcMouseOver, true);
      Util$1.addEvent(document, 'mouseout', pcMouseOut, true);
    } else {
  ...
  function visitorInit(config) {
    ...
    if (Util$1.paramType(window.top) !== 'Undefined' && window.top !== window.self && (window.location.href.indexOf('visual=true') > -1 || Storage$1.getSession('visitor') === true)) {
      loadVisitorSDK();
    } else {
  ...
  var startDate = +new Date(); // 获取浏览器窗口的可视区域的宽度
  ...
  function getDuration() {
    ...
    if (Util$1.paramType(baseConfig.base.webstayDuration) === 'Number' && baseConfig.base.webstayDuration > 0) {
      if (duration > baseConfig.base.webstayDuration) {
  ...
  function loadHeatmapSDK() {
    ...
    Util$1.addScript('AnalysysAgent_JS_SDK_HEATMAP', heatmapConfig.SDKFileDirectory);
    window.ARK_HEATMAP = {
  ...
  function addClickEvent(event) {
    ...
    if (Util$1.checkTypeList(heatmapConfig.heatMapBlackList, ele) || heatmapConfig.heatMapWhiteList && !Util$1.checkTypeList(heatmapConfig.heatMapWhiteList, ele)) return; // e.stopPropagation()
    // e.preventDefault()
  ...
  var hybridAns = {
    ...
    alias: function (aliasID, trackID, callback) {
      if (Util$1.paramType(trackID) === 'Function') {
        callback = trackID;
        trackID = '';
      }
    ...
    pageView: function (pageName, pageInfo, callback) {
      if (Util$1.checkTypeList(baseConfig.base.pageViewBlackList) || baseConfig.base.pageViewWhiteList && !Util$1.checkTypeList(baseConfig.base.pageViewWhiteList)) return;
      ...
      if (Util$1.paramType(pageName) === 'String') {
        pageInfo.$title = pageName;
      }

      if (window.AnalysysModule && window.AnalysysModule.pageClose && Util$1.paramType(window.AnalysysModule.pageClose.createTime) === 'Function') {
  ...
  function loadIframeUrl(url, callback) {
    ...
    if (Util$1.paramType(callback) === 'Function') {
  ...
  function transporter(funName, paramArray, callbackFunName, callback) {
    ...
    if (Util$1.paramType(callbackFunName) === 'String') {
      params.callbackFunName = callbackFunName;
    } else if (Util$1.paramType(callbackFunName) === 'Function') {
  ...
  function backParamsArray() {
    ...
    for (var i = 0; i < arg.length; i++) {
      if (arg[i] !== undefined) {
        if (Util$1.paramType(arg[i]) === 'Object') {
          for (var key in arg[i]) {
            if (Util$1.paramType(arg[i][key]) === 'Function') {
              arg[i][key] = arg[i][key].call(arg[i][key]);
            }
          }
        }

        if (Util$1.paramType(arg[i]) === 'Function') {
          callback = arg[i];
        } else {
          argArray.push(arg[i]);
        }
  ...
  function hashView() {
    ...
    pageHash = window.location.href; // var pageProperty = { $url: pageHash, $title: document.title }
    // var paramArray = backParamsArray('', pageProperty)
    // transporter('pageView', paramArray.argArray)
  ...
  var fieldTemplate$5 = {
    $user_click: {
      xcontext: ['$page_width', '$page_height', '$url_path', '$url', '$title', '$is_first_day', '$session_id', '$is_time_calibrated', '$device_type', '$element_type', '$element_path', '$element_content', '$element_id', '$element_name', '$element_target_url', '$element_class_name']
    }
  };

  var userClickConfig = {};

  function addClickEvent$1(event) {
    var e = event || window.event;
    ...
    var ele = e.target || e.srcElement;
    if (Util$1.checkTypeList(userClickConfig.autoClickBlackList, ele) || userClickConfig.autoClickWhiteList && !Util$1.checkTypeList(userClickConfig.autoClickWhiteList, ele)) return;
    elePostion.ele = ele;
    getParentClickableElement();
  ...
  function errorLog() {
    ...
    if ((errorCode === '600017' || errorCode === '600019' || errorCode === '60009' || errorCode === '600010' || errorCode === '60005') && Util$1.paramType(value) === 'String' && value.length && value.length > 30) {
      value = Util$1.stringSlice(value, 30) + '...';
    }

    if (Util$1.paramType(key) === 'String' && key.length > 30) {
      key = Util$1.stringSlice(key, 30) + '...';
    }

    if (Util$1.paramType(fn) === 'String' && fn.length > 30) {
      fn = Util$1.stringSlice(fn, 30) + '...';
    }

    var msgTemp = errorMessage[errorCode] || errorMessage.common;
    var showMsg = msgTemp.replace('{FN}', fn).replace('{KEY}', key).replace('{VALUE}', value).replace('{VALUETYPE}', valueType);
  ...
  function checkFields(key, value, rule) {
    ...
    var checkValue = checkRule.value || fieldRules$7.xcontextCommonRule.value;
      ...
      for (var key in obj) {
        ...
        if (!valueStatus) {
          if (baseConfig.status.errorCode === '600019') {
            if (Util$1.paramType(obj[key]) === 'String' && obj[key].length > 500) {
  ...
  function MoveBox() {
    ...
    function move(event) {
      ...
      _this.parent.style.top = y + 'px'; // _this.parentOption = {
      //     x: x,
      //     y: y
      // }
    }
  ...
  function openVisualBox(ele) {
    ...
    var tagName = ele.tagName; // var elePosition = boxPosition(ele)
    ...
    if (eventAttri) {
      config = Util$1.objMerge(config, JSON.parse(eventAttri));
      config.isChange = true;
      config.link = elePath$1;
      config.index = eleIndex$1;
    } else {
  ...
  function setVisualEvent(ele, config) {
    ...
    if (eleClassName) {
      eleClassName = eleClassName.replace(' ARK_SAVE_NO_DISPOSE', '').replace(' ARK_SAVE_DISPOSE', '').replace(' ARK_SAVE_CHANGE_DISPOSE', '');
    }
  ...
  function removeVisualEvent(ele) {
    ...
    if (eleClassName) {
      ele.className = eleClassName.replace(' ARK_SAVE_NO_DISPOSE', '').replace(' ARK_SAVE_DISPOSE', '').replace(' ARK_SAVE_CHANGE_DISPOSE', '');
    }
  }
  ...
  function callbackMouseMove() {
    ...
    elePath$1 = domParentList(mouseMoveEle);

    if (patt.test(elePath$1) === true) {
      elePath$1 = elePath$1.replace(patt, '');
    }

    eleIndex$1 = setIndex(mouseMoveEle, elePath$1);
  ...
  function offset$1(curEle) {
    var totalLeft = null;
    var totalTop = null;
    var par = curEle.offsetParent; // 首先加自己本身的左偏移和上偏移
  ...
  function mouseMoveEvent(e) {
    ...
    if (ele !== eleDiv && mouseMoveEle !== ele) {
      ...
      var y = offset$1(mouseMoveEle).top - offset$1(eleOffsetParent).top;
      var x = offset$1(mouseMoveEle).left - offset$1(eleOffsetParent).left;
      ...
      Util$1.addEvent(eleDiv, 'click', callbackMouseMove); // checkChildrenEvent(mouseMoveEle)
    }
  ...
  function addVisualListener() {
    ...
    Util$1.addEvent(document, 'mouseover', showEleHover, true);
  }

  function removeVisualListener() {
    delClickBox();
    Util$1.removeEvent(document, 'mouseover', showEleHover, true);
  ...
  function hiddenVisualEvent(status) {
    delClickBox();
    var eleList = Util$1.selectorAllEleList('[data-ark-attr]'); // document.querySelectorAll('[data-ark-attr]')
  ...
  var useragentBase = function () {
    ...
    var STRINGS_SHARP = 'Sharp';
    var STRINGS_SONY_ERICSSON = 'Sony Ericsson';
    var STRINGS_MOTOROLA = 'Motorola';
    var STRINGS_LG = 'LG';
    var STRINGS_HUAWEI = 'Huawei';
    var STRINGS_HTC = 'HTC';
    var STRINGS_COOLPAD = 'Coolpad';
    var STRINGS_ASUS = 'Asus';
    var STRINGS_ACER = 'Acer';
    var match = '';
    var TOUCHWIZ_MODELS = {
      SAMSUNG: {
  ...
  var useragentBase$1 = {
    __proto__: null,
    'default': useragentBase,
    __moduleExports: useragentBase
  };

  var UA$1 = ( useragentBase$1 && useragentBase ) || useragentBase$1;

  var uaDevice = function (ua) {
    var uaData = new UA$1(ua);
    var match;
    var tmpMatch; // handle mobile device
    ...
    if (uaData.device.type === 'mobile' || uaData.device.type === 'tablet') {
      ...
      // 三星手机类型：galaxy xxx|SM-xxx|GT-xxx|SCH-xxx|SGH-xxx|SPH-xxx|SHW-xxx  若这些均未匹配到，则启用在中关村在线爬取到的机型白名单进行判断
          ...
          uaData.device.manufacturer = 'Samsung'; // 解决移动联通等不同发行版导致的机型不同问题
          ...
          // 华为机型特征：Huawei[\s-_](\w*[-_]?\w*)  或者以 7D-  ALE-  CHE-等开头
            ...
            // hongmi有特殊判断build/hm2015011
              ...
              // vivo机型特征: Vivo[\s-_](\w*)  或者以 E1  S11t  S7t 等开头
                  ...
                  uaData.device.model = match[1]; // 首先剔除 viv-  vivo-  bbg- 等打头的内容

                  uaData.device.model = uaData.device.model.replace(/(viv[\s-_]|vivo[\s-_]|bbg[\s-_])/i, ''); // 解决移动联通等不同发行版导致的机型不同问题
      ...
      if (uaData.device.model) {
        ...
        uaData.device.model = uaData.device.model.match(/\s*(\w*\s*\w*)/)[1].replace(/\s+/, '-'); // 针对三星、华为做去重的特殊处理
    ...
    if (match = ua.match(/baiduboxapp\/?([\d\.]*)/i)) {
      ...
      uaData.browser.name = '轻应用runtime'; // uaData.browser.name = 'qing runtime';
    } else if (/Weibo/i.test(ua)) {
      uaData.browser.name = '微博'; // uaData.browser.name = 'weibo';
    } else if (/MQQ/i.test(ua)) {
      uaData.browser.name = '手机QQ'; // uaData.browser.name = 'mobile qq';
  ...
  var parser = new uaDevice(navigator.userAgent);
  var os = parser.os.name || '';
  ...
  function openDebugBox(obj) {
    ...
    var config = {
      top: elePosition$1.top,
      left: elePosition$1.left,
      list: obj.event_info || []
    };
  ...
  var mouseMoveEle$1 = null;
  var elePosition$1 = null;
  var elePath$2 = null;
  var patt$1 = /\d{13}/g;

  function callbackMouseMove$1() {
    elePosition$1 = boxPosition(mouseMoveEle$1, 'debug');

    if (patt$1.test(elePath$2) === true) {
      elePath$2 = elePath$2.replace(patt$1, '');
    }

    sendDebugMsg(mouseMoveEle$1);
  }

  function offset$2(curEle) {
    var totalLeft = null;
    var totalTop = null;
    var par = curEle.offsetParent; // 首先加自己本身的左偏移和上偏移
  ...
  function mouseMoveEvent$1(e) {
    var ele = e.target;
    var tagName = ele.tagName;
    eleDiv$1 = document.getElementById('ARK_CLICK');

    if (eleDiv$1 && (Math.abs(e.clientX - moveX$1) > 3 || Math.abs(e.clientY - moveY$1) > 3)) {
      eleDiv$1.style.display = 'none';
    }
    ...
    if (ele !== eleDiv$1 && mouseMoveEle$1 !== ele) {
      mouseMoveEle$1 = ele;

      if (eleDiv$1) {
        var parentEle = eleDiv$1.parentNode || document.body;
        parentEle.removeChild(eleDiv$1);
      }

      eleDiv$1 = document.createElement('div');
      eleDiv$1.id = 'ARK_CLICK';
      eleDiv$1.className = 'ARK_CLICK_ELE';
      var eleOffsetParent = mouseMoveEle$1.offsetParent || document.body;
      var y = offset$2(mouseMoveEle$1).top - offset$2(eleOffsetParent).top;
      var x = offset$2(mouseMoveEle$1).left - offset$2(eleOffsetParent).left;
      var width = mouseMoveEle$1.offsetWidth;
      var height = mouseMoveEle$1.offsetHeight;
      eleDiv$1.style.top = y + 'px';
      eleDiv$1.style.left = x + 'px';
      eleDiv$1.style.width = width + 'px';
      eleDiv$1.style.height = height + 'px';
      eleParent.appendChild(eleDiv$1);
      Util$1.addEvent(eleDiv$1, 'click', callbackMouseMove$1); // checkChildrenEvent(mouseMoveEle)
    }
  ...
  function addDebugListener() {
    delDebugBox();
    Util$1.addEvent(document, 'mousemove', mouseMoveEvent$1);
    Util$1.addEvent(document, 'mouseover', setDebugHover); // Util.addEvent(document, 'mouseover', removeDebugHover)
  }

  function removeDebugListener() {
    delDebugBox();
    Util$1.removeEvent(document, 'mouseover', setDebugHover);
    Util$1.removeEvent(document, 'mousemove', mouseMoveEvent$1);
    var hoverEle = document.getElementById('ARK_CLICK');
  ...
  function initVisual() {
    if (isEmbedded('visual') || Storage$1.getSession('ISVISUAL') === true) {
      getMsg(processMsg);
      Storage$1.setSession('ISVISUAL', true);
    }
  }
